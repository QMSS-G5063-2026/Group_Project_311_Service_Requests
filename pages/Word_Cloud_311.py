...
# Cascading ZIP based on neighborhood
...
zips = ["All Zips"] + sorted([z for z in relevant_zips if z.isdigit() and len(z) == 5])
...
# Specific Issue (cascading from category)
...
color_options = {
    "🔴 Red–Blue":    "RdYlBu",
    "🟠 Orange–Purple": "plasma",
    "🟢 Green":       "Greens",
    "🔵 Blue":        "Blues",
    "🌈 Rainbow":     "rainbow",
    "⚫ Greyscale":   "Greys",
}
selected_color_label = st.sidebar.selectbox("Color Theme", list(color_options.keys()))
selected_colormap = color_options[selected_color_label]
...
if selected_zip != "All Zips":
    filtered = filtered[filtered["Incident Zip"] == selected_zip]
if selected_group != "All Categories":
...
st.markdown(
    ...
    f"colored with **{selected_color_label}**"
)
...
if not filtered.empty:
    st.markdown("---")
    st.subheader("📋 Top 10 Complaints")
    top10 = (
        filtered["Complaint"]
        .value_counts()
        .head(10)
